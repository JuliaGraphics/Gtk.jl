...
function gen_g_type_lists(gtk_h)
    ...
    for tdecl in tdecls
        ...
        elseif gtyp == 0 || g_isa(gtyp, g_type_from_name(:GBoxed))
            ...
            has_ref_fn = false
            if get_fn_ptr(unref_fn, libname, Gtk) == C_NULL
                ...
                if get_fn_ptr(unref_fn, libname, Gtk) == C_NULL
                    ...
                    @assert get_fn_ptr(ref_fn_sym, libname, Gtk) != C_NULL
                    ref_fn = :(ccall(($(QuoteNode(ref_fn_sym)),$libname),Nothing,(Ptr{Nothing},),ref))
                end
            else
                ref_fn = nothing
            end
            if length(children(sdecl)) == 0 || has_ref_fn || isptr
                # Opaque box
                if unref_fn === nothing
                ...
                for cu in children(sdecl)
                    name = spelling(cu)
                    if isempty(name)
                        name = gensym()
                    end
                    jtyp = g_type_to_jl(type(cu))
                    if jtyp === :Nothing
                        jtyp = " < $(type(cu)) >"
                        valid = false
                    end
